...
app.post('/generate-article', async (req, res) => {
  try {
    ...
    if (prompts && prompts.article && prompts.article.template) {
      ...
      // デフォルトプロンプト
      promptTemplate = `あなたは技術記事の専門ライターです。YouTube動画の内容をもとに、動画で述べられている内容のみを使用して解説記事を作成してください。

{{templateNote}}

⚠️ 重要な制約:
- 動画の文字起こし・要約に含まれている内容のみを使用してください
- 動画で言及されていない情報や一般的な知識は追加しないでください
- 動画で述べられている事実、手順、説明のみに基づいて記事を構成してください
- タイムスタンプがある場合は必ず含めて、読者が該当箇所を確認できるようにしてください

記事の構成要件:
1. **タイトル**: 動画の内容を反映したタイトル
2. **導入部**: 動画で説明されている概要
3. **本文**: 動画の流れに沿った論理的な構成
4. **手順・説明**: 動画で実際に示された手順や解説
5. **まとめ**: 動画で述べられた要点の整理
